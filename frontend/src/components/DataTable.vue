...
async function addItem() {
  ...
  if (
    currentTop.contact_name !== tempItem.contact_name ||
    currentTop.contact_email !== tempItem.contact_email
  ) {
    tableData.value.unshift({
      contact_name: "New Item",
      contact_email: "New Item",
    });
    currentTop = tempItem;
    const config = {
      method: "put",
        ...
        <tr v-for="(item, index) in tableData.values()">
          <th scope="row">
            <input
              ...
              @focusout="updateContact(item.contact_email, 'contact_email')"
              @focusin="storeCurrentItem(item)"
            />
          </th>
          <td>
            <input
              ...
              @focusout="updateContact(item.contact_name, 'contact_name')"
              @focusin="storeCurrentItem(item)"
            />
          </td>
          <td>
            <button @click="deleteItem(item.contact_email, index)" class="btn">
